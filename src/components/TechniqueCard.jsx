...
export default function TechniqueCard({ tech, locked }) {
  ...
  return (
    <div className={`max-w-[330px] w-full bg-[#FFFAFA] border border-gray-200 shadow-md rounded-2xl overflow-hidden transform transition-all duration-300 hover:border-red-600 hover:shadow-lg ${locked ? "opacity-50 cursor-not-allowed" : ""}`}>
      ...
      <div className="flex flex-col p-5">
        <div className="flex justify-between items-start mb-2">
          ...
          <button onClick={() => toggleFavorite(tech.id)} className={`text-xl p-2 rounded-lg border-2 border-[#C6C6C6] bg-white hover:scale-90 ${locked ? "opacity-50 cursor-not-allowed" : ""} ${favorited ? "text-red-600" : "text-gray-500"}`}>
        ...
        {(
          <div className="flex flex-col gap-2">
            <button onClick={() => toggleInProgress(tech.id)} className={`w-full py-1 bg-blue-100 text-blue-700 font-semibold rounded-lg  ${locked ? "opacity-70 cursor-not-allowed" : ""}`}>
